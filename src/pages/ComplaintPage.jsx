...
import {
  ...
  AlertTriangle, Eye, Loader2, Sparkles, User, Mail,
  Store, Bot, BarChart3, TrendingUp, XCircle,
  Upload, ArrowRight, Info, Shield, ClipboardList, Activity,
  Siren, MessageSquare, ChevronDown, X,
} from 'lucide-react';
...
const ALL_STATUSES = ['Submitted', 'Under Review', 'Inspection Scheduled', 'Action Taken', 'Resolved'];
...
function storeComplaints(complaints) {
      ...
      <div className="sticky top-16 z-30 glass border-b border-border/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex gap-1 py-2 overflow-x-auto no-scrollbar">
            {([
              { key: 'submit', label: '📝 Submit Complaint', icon: FileText },
              { key: 'track', label: '📊 Track Complaint', icon: Search },
              { key: 'hotspots', label: '📍 Safety Hotspots', icon: MapPin },
...
function SubmitComplaintTab() {
  ...
  const handlePhotoUpload = (e) => {
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = ev => {
        if (ev.target?.result) {
          setPhotos(prev => [...prev, ev.target.result].slice(0, 5));
        }
      };
  ...
  const analyzeWithAI = async (description, category) => {
    try {
...
function SubmitSuccess({ complaint, onNew }) {
  return (
