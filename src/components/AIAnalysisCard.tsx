...
interface Props {
  ...
  aiHarmfulIngredients?: HarmfulIngredient[];
  loading?: boolean;
  onRetry?: () => void;
}

/* ─── parser ─────────────────────────────────────────── */
function parseAISummary(raw: string): ParsedAnalysis {
...
function HarmfulIngredientCard({ item, index }: { item: HarmfulIngredient; index: number }) {
  const [open, setOpen] = useState(false);
